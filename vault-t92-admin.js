...
async function loadProperties() {
  ...
  try {
      ...
      propertyList.innerHTML += `
        <div class="property-item">
          <div>
            ...
            <p>📸 ${property.images?.length || 0} images</p>
          </div>
          <div style="display:flex; gap:10px;">
            <button
              style="background:black; color:white; padding:10px 15px; border:none; border-radius:8px; cursor:pointer;"
              onclick="editProperty('${docSnap.id}')">
              Edit
            </button>
            <button class="delete-btn" onclick="deleteProperty('${docSnap.id}')">
              Delete
            </button>
          </div>
        </div>
    ...
    console.error(error);
    propertyList.innerHTML = "Error loading properties.";
  }
}
...
uploadBtn.addEventListener("click", async () => {
  try {
    ...
    loadingText.innerText = "Uploading property...";
    ...
    // VALIDATION
    if (!title || !location || !price || !bedrooms || !bathrooms || !size || !description || imageFiles.length === 0) {
      alert("Please complete all fields and select images");
    ...
    for (let i = 0; i < imageFiles.length; i++) {
      loadingText.innerText = `Uploading image ${i + 1} of ${imageFiles.length}`;
...
updateBtn.addEventListener("click", async () => {
  try {
    loadingOverlay.style.display = "flex";
    loadingText.innerText = "Updating property...";
    ...
    snapshot.forEach((docSnap) => {
      ...
      bookingList.innerHTML += `
        <div class="booking-item">
          ...
          <button class="delete-booking-btn" onclick="deleteBooking('${docSnap.id}')">
            Delete Booking
          </button>
        </div>
      `;
